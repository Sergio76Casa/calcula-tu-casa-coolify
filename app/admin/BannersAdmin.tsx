...
export default function BannersAdmin() {
  ...
  return (
    <section className="bg-slate-900 border border-white/10 rounded-2xl p-5">
      ...
      {error && (
        <div className="p-4 text-sm text-red-400 bg-red-500/10 border border-red-500/20 rounded-xl mt-4">
          {error}
        </div>
      )}
          ...
          {loading ? (
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
              ...
              {banners.map(b => (
                <div
                  key={b.id}
                  className="flex items-center gap-4 bg-slate-950 border border-white/5 rounded-2xl px-4 py-3"
                >
                  <button
