...
async function geminiGeocoding(
  direccion: string,
  apiKey: string
...
Responde SOLO con el JSON.`;
  ...
  try {
    ...
    console.error("[api/valorar] Gemini Geocoding fallback error:", err);
    return null;
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    if (!propiedadId) {
      ...
      try {
        ...
        const geoRes = await fetch(url, {
          headers: { "User-Agent": "CalculaTuCasa-Tasador/1.2 (tasaciones@calculatucasa.com)" },
        ...
        console.error("[Geocoding error Nominatim]", err);
      }

      // Fallback a geocodificación con Gemini
      if (coordLat === null || coordLon === null) {
        console.log("[Geocoding] Nominatim falló. Usando fallback con Gemini para:", propiedad.direccion_completa);
        const fallbackCoords = await geminiGeocoding(propiedad.direccion_completa, GEMINI_API_KEY);
        if (fallbackCoords) {
          coordLat = fallbackCoords.lat;
          coordLon = fallbackCoords.lon;
          console.log(`[Geocoding] Gemini fallback exitoso: lat=${coordLat}, lon=${coordLon}`);
        }
      }
    ...
    return NextResponse.json({
      ...
      score_inversion,
      coordenadas: coordLat && coordLon ? { lat: coordLat, lon: coordLon } : null,
    });
  } catch (err) {
